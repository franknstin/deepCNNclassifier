...
class DataIngestion:
    ...
    def download_file(self):
        ...
        if not os.path.exists(self.config.local_data_file):
            logger.info('download sarted')
            filename, headers = request.urlretrieve(
                url=self.config.source_URL,
                filename=self.config.local_data_file
            )
            logger.info(f"{filename} download! wiht following info: \n{headers}")
        else:
            logger.info(f"file already exits and is of size: {get_size(Path(self.config.local_data_file))}")
